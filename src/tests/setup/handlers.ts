...
export const handlers = [
  ...
  http.post("https://jsonplaceholder.typicode.com/posts", async (req, res, ctx) => {
    const body = await req.json();
    return res(ctx.status(201), ctx.json({ id: 101, ...body }));
  }),

  http.put("https://jsonplaceholder.typicode.com/posts/1", async (req, res, ctx) => {
    const body = await req.json();
    return res(ctx.status(200), ctx.json({ id: 1, ...body }));
  }),

  http.delete("https://jsonplaceholder.typicode.com/posts/1", (req, res, ctx) => {
